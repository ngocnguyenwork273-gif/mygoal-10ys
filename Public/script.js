...
async function batDauDongBo(user) {
  if (kenhRealtime) { await supabaseClient.removeChannel(kenhRealtime); kenhRealtime = null; }
  dangTaiLanDau = true;
...
// ===== Phân cách hàng nghìn cho ô nhập số (kiểu VN: dấu chấm, vd 10.000) =====
function phanCachHangNghin(chuoi) {
  const soThuan = String(chuoi).replace(/\D/g, '');
  if (!soThuan) return '';
  return soThuan.replace(/\B(?=(\d{3})+(?!\d))/g, '.');
}
function ganPhanCachHangNghin(input) {
  input.addEventListener('input', function () {
    const soChuSoTruocCon = this.value.slice(0, this.selectionStart).replace(/\D/g, '').length;
    this.value = phanCachHangNghin(this.value);
    let dem = 0, viTriMoi = this.value.length;
    for (let i = 0; i < this.value.length; i++) {
      if (/\d/.test(this.value[i])) dem++;
      if (dem === soChuSoTruocCon) { viTriMoi = i + 1; break; }
    }
    this.setSelectionRange(viTriMoi, viTriMoi);
  });
}
ganPhanCachHangNghin(document.getElementById('chi-tieu-gio'));
...
function suaMucTieu(id) {
  ...
  document.getElementById('chi-tieu-gio').value = phanCachHangNghin(m.chiTieu);
...
document.getElementById('btn-them-muc-tieu').addEventListener('click', () => {
  ...
  const chiTieu = parseInt(document.getElementById('chi-tieu-gio').value.replace(/\D/g, ''), 10);
